...
public extension TelegramEngine {
    final class Messages {
        ...
        public func markStoryAsSeen(peerId: EnginePeer.Id, id: Int32, asPinned: Bool) -> Signal<Never, NoError> {
            ...
            // MARK: Swiftgram — ghost mode: не отправляем просмотр историй (AyuGram: sendReadStories)
            // .never() безопасен: не часть retry-машины состояний
            if SGSimpleSettings.shared.isGhostModeEnabled && !SGSimpleSettings.shared.ghostModeSendReadStories {
                return .never()
            }
...
// MARK: Swiftgram
private func sgTranslateWithBackend(text: String, toLang: String) -> Signal<String, TranslateFetchError>? {
    let backend = SGSimpleSettings.shared.translationBackend
    if backend == SGSimpleSettings.TranslationBackend.yandex.rawValue {
        return syandex(text, toLang)
    }
    if backend == SGSimpleSettings.TranslationBackend.google.rawValue {
        return sgoogle(text, toLang)
    }
    return nil
}
...
private func sgWrappedTranslateSingle(
    ...
    if let backendSignal = sgTranslateWithBackend(text: text, toLang: toLang) {
...
private func sgWrappedTranslateMultiple(
    ...
    if texts.first?.0 != nil, sgTranslateWithBackend(text: texts.first!.0, toLang: toLang) != nil {
