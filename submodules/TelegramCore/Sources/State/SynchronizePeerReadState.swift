...
private func pushPeerReadState(network: Network, postbox: Postbox, stateManager: AccountStateManager, peerId: PeerId) -> Signal<Never, PeerReadStateValidationError> {
    // MARK: Swiftgram — ghost mode: не отправляем read-реквизиты (AyuGram: sendReadMessages,
    // data_histories.cpp:700 — if (!sendReadMessages()) return;)
    // .complete() обязателен, .never() заклинит ManagedSynchronizePeerReadStates (D1)
    if SGSimpleSettings.shared.isGhostModeEnabled && !SGSimpleSettings.shared.ghostModeSendReadMessages {
        return .complete()
    }
