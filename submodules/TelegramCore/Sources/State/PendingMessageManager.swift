...
public final class PendingMessageManager {
    ...
    private func sendGroupMessagesContent(network: Network, postbox: Postbox, stateManager: AccountStateManager, accountPeerId: PeerId, group: [(messageId: MessageId, content: PendingMessageUploadedContentAndReuploadInfo)]) -> Signal<Void, NoError> {
        ...
        return postbox.transaction { [weak self] transaction -> Signal<Void, NoError> in
            ...
            if peerId.namespace == Namespaces.Peer.SecretChat {
                ...
                // MARK: Swiftgram — ghost mode: sendWithoutSound — silent-флаг
                // (AyuGram: SendWithoutSoundOption — Never/InGhostMode/Always)
                if (SGSimpleSettings.shared.isGhostModeEnabled && SGSimpleSettings.shared.ghostModeSendWithoutSoundEnum == .inGhostMode) || SGSimpleSettings.shared.ghostModeSendWithoutSoundEnum == .always {
                    flags |= Int32(1 << 2)
                }
    ...
    // MARK: Swiftgram — ghost mode: markReadAfterAction — принудительное чтение в обход гейта
    private func forceReadHistoryAfterAction(network: Network, postbox: Postbox, peerId: PeerId) {
        let _ = (postbox.transaction { transaction -> (Api.InputPeer, Int32)? in
            guard let peer = transaction.getPeer(peerId), let inputPeer = apiInputPeer(peer) else {
                return nil
            }
            guard let topIndex = transaction.getTopPeerMessageIndex(peerId: peerId) else {
                return nil
            }
            return (inputPeer, topIndex.id.id)
        }
        |> mapToSignal { result -> Signal<Never, NoError> in
            guard let (inputPeer, maxId) = result else {
                return .complete()
            }
            let request: Signal<Never, MTRpcError>
            switch inputPeer {
            case let .inputPeerChannel(channelData):
                request = network.request(Api.functions.channels.readHistory(channel: .inputChannel(.init(channelId: channelData.channelId, accessHash: channelData.accessHash)), maxId: maxId))
                |> ignoreValues
            default:
                request = network.request(Api.functions.messages.readHistory(peer: inputPeer, maxId: maxId))
                |> ignoreValues
            }
            return request
    ...
    private func applySentMessage(postbox: Postbox, stateManager: AccountStateManager, message: Message, content: PendingMessageUploadedContentAndReuploadInfo, result: Api.Updates) -> Signal<Void, NoError> {
        // MARK: Swiftgram — ghost mode: markReadAfterAction — после успешной отправки
        // принудительно читаем историю, если read-реквизиты подавлены
        // (AyuGram: telegram_helpers.cpp:449-457 — срабатывает только при sendReadMessages==false)
        if SGSimpleSettings.shared.isGhostModeEnabled && !SGSimpleSettings.shared.ghostModeSendReadMessages && SGSimpleSettings.shared.ghostModeMarkReadAfterAction {
            self.forceReadHistoryAfterAction(network: self.network, postbox: postbox, peerId: message.id.peerId)
        }
