    ...
    for operation in optimizedOperations(finalState.state.operations) {
        switch operation {
            ...
            case let .DeleteMessagesWithGlobalIds(ids):
                // MARK: Swiftgram — anti-delete: saveDeletedMessages (AyuGram: processMessageDelete)
                if SGSimpleSettings.shared.saveDeletedMessages {
                    ...
                    for messageId in transaction.messageIdsForGlobalIds(ids) {
                        ...
                        if let peer = transaction.getPeer(messageId.peerId) as? TelegramUser, peer.botInfo != nil {
                            shouldSave = SGSimpleSettings.shared.saveForBots
                        }
                        if shouldSave {
                            transaction.updateMessage(messageId, update: { message in
                                var attributes = message.attributes
                                attributes.removeAll(where: { $0 is DeletedMessageAttribute })
                                attributes.append(DeletedMessageAttribute(date: Int32(Date().timeIntervalSince1970)))
            ...
            case let .DeleteMessages(ids):
                // MARK: Swiftgram — anti-delete: канальные удаления (updateDeleteChannelMessages → processMessagesDeleted в AyuGram). Scheduled/ephemeral/quick-reply не трогаем.
                if SGSimpleSettings.shared.saveDeletedMessages {
                    var cloudIds: [MessageId] = []
                    var otherIds: [MessageId] = []
                    for id in ids {
                    ...
                    for messageId in cloudIds {
                        ...
                        if shouldSave {
                            ...
                            otherIds.append(messageId)
                        }
                    }
                    if !otherIds.isEmpty {
                        _internal_deleteMessages(transaction: transaction, mediaBox: mediaBox, ids: otherIds, manualAddMessageThreadStatsDifference: { id, add, remove in
                            addMessageThreadStatsDifference(threadKey: id, remove: remove, addedMessagePeer: nil, addedMessageId: nil, isOutgoing: false)
                        })
                        deletedMessageIds.append(contentsOf: otherIds.map { .messageId($0) })
                    }
                } else {
            ...
            case let .EditMessage(id, message):
                ...
                transaction.updateMessage(id, update: { previousMessage in
                    ...
                    // MARK: Swiftgram — anti-delete: saveMessagesHistory (AyuGram: addEditedMessage)
                    if SGSimpleSettings.shared.saveMessagesHistory, previousMessage.text != message.text {
                        let existing = previousMessage.attributes.first(where: { $0 is EditedHistoryMessageAttribute }) as? EditedHistoryMessageAttribute
                        let revision = EditHistoryRevision(text: previousMessage.text, entities: previousMessage.textEntitiesAttribute?.entities ?? [], date: previousMessage.timestamp)
                        var revisions = existing?.revisions ?? []
                        revisions.insert(revision, at: 0)
                        if revisions.count > 10 {
                            revisions.removeLast(revisions.count - 10)
                        }
                        updatedAttributes.removeAll(where: { $0 is EditedHistoryMessageAttribute })
                        updatedAttributes.append(EditedHistoryMessageAttribute(revisions: revisions))
                    }
            ...
            case let .ReadOutbox(messageId, timestamp):
                ...
                // MARK: Swiftgram — spy: сообщение прочитано собеседником (AyuGram: SpyMessageRead)
                if SGSimpleSettings.shared.spyMessageRead, messageId.peerId != accountPeerId {
                    SGSpyStorage.shared.record(kind: .read, peerId: messageId.peerId.id._internalGetInt64Value(), messageId: messageId.id, date: timestamp ?? Int32(Date().timeIntervalSince1970))
                }
            ...
            case let .ReadMessageContents(peerIdAndMessageIds, date):
                ...
                if let peerId = peerId {
                    // MARK: Swiftgram — spy: содержимое сообщения открыто (AyuGram: SpyMessageContentsRead)
                    if SGSimpleSettings.shared.spyMessageContentsRead, peerId != accountPeerId {
                        for id in messageIds {
                            SGSpyStorage.shared.record(kind: .contentsRead, peerId: peerId.id._internalGetInt64Value(), messageId: id, date: date ?? Int32(Date().timeIntervalSince1970))
                        }
                    }
                    for id in messageIds {
                    ...
                    // MARK: Swiftgram — spy: содержимое сообщения открыто (globalIds)
                    if SGSimpleSettings.shared.spyMessageContentsRead {
                        for messageId in transaction.messageIdsForGlobalIds(messageIds) {
                            if messageId.peerId != accountPeerId {
                                SGSpyStorage.shared.record(kind: .contentsRead, peerId: messageId.peerId.id._internalGetInt64Value(), messageId: messageId.id, date: date ?? Int32(Date().timeIntervalSince1970))
                            }
                        }
                    }
                    for messageId in transaction.messageIdsForGlobalIds(messageIds) {
