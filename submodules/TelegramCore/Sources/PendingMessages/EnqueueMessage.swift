        ...
        for (transformedMedia, message) in updatedMessages {
            ...
            switch message {
                case let .message(text, requestedAttributes, inlineStickers, mediaReference, threadId, replyToMessageId, replyToStoryId, localGroupingKey, _, _):
                    ...
                    var sendAsPeer: Peer?
                    // MARK: Swiftgram — ghost mode: useScheduledMessages — отправка как запланированные
                    // (AyuGram: applyGhostScheduling, telegram_helpers.cpp:1578-1586, дефолт 12 сек)
                    if SGSimpleSettings.shared.isGhostModeEnabled && SGSimpleSettings.shared.ghostModeUseScheduledMessages, !(peer is TelegramSecretChat) {
                        let hasSchedule = attributes.contains(where: { $0 is OutgoingScheduleInfoMessageAttribute })
                        if !hasSchedule {
                            attributes.append(OutgoingScheduleInfoMessageAttribute(scheduleTime: timestamp + 12, repeatPeriod: nil))
                        }
                    }
                    for attribute in attributes {
