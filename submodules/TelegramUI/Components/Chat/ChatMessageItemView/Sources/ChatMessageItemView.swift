...
open class ChatMessageItemView: ListViewItemNode, ChatMessageItemNodeProtocol {
    ...
    open func setupItem(_ item: ChatMessageItem, synchronousLoad: Bool) {
        ...
        self.applySGRegexFiltersIfNeeded(item)
        self.wasFilteredKeywordTested = true
    }
    
    private func applySGRegexFiltersIfNeeded(_ item: ChatMessageItem) {
        let settings = SGSimpleSettings.shared
        guard !item.message.text.isEmpty, item.message.effectivelyIncoming(item.context.account.peerId) else {
            return
        }
        let peerId = item.message.id.peerId
        let fadeAlpha = item.presentationData.theme.theme.overallDarkAppearance ? 0.2 : 0.3
        if settings.hideFromBlocked {
            if let authorId = item.message.author?.id, SGFiltersBlockedPeers.shared.isBlocked(authorId, context: item.context) {
                self.matchedFilterKeyword = "blocked"
                self.alpha = fadeAlpha
                return
            }
        }
        guard settings.filtersEnabled else {
            return
        }
        let isChannel: Bool
        ...
        if !settings.filtersEnabledInChats && !isChannel {
            return
        }
        if SGFilterEngine.shared.isFiltered(text: item.message.text, dialogId: peerId.id._internalGetInt64Value()) {
            self.matchedFilterKeyword = "filter"
            self.alpha = fadeAlpha
        }
    }
