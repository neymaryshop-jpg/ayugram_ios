...
public class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
    ...
    private static func applyLayout(selfReference: Weak<ChatMessageBubbleItemNode>,
        ...
        // MARK: Swiftgram — anti-delete: полупрозрачность удалённых (AyuGram: Element::deletedOpacity, 0.7)
        if SGSimpleSettings.shared.semiTransparentDeletedMessages, item.message.attributes.contains(where: { $0 is DeletedMessageAttribute }) {
