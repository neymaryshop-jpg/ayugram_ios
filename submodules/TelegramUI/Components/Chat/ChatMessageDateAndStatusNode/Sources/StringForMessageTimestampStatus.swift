...
public func stringForMessageTimestampStatus(
    ...
    // MARK: Swiftgram — anti-delete: deletedMark приписывается перед именем/датой (AyuGram: history_view_bottom_info.cpp:530 deleted + name + afterAuthor)
    if message.attributes.contains(where: { $0 is DeletedMessageAttribute }) {
        let mark = SGSimpleSettings.shared.deletedMark
        if !mark.isEmpty {
            dateText = "\(mark) \(dateText)"
        }
    }
    
    // MARK: Swiftgram — spy: плашка "прочитано" / "содержимое открыто" (AyuGram: SpyMessageRead / SpyMessageContentsRead)
    if message.id.peerId != context.account.peerId {
        let peerId = message.id.peerId.id._internalGetInt64Value()
        if let readDate = SGSpyStorage.shared.date(kind: .read, peerId: peerId, messageId: message.id.id) {
            dateText = "\(dateText) \(strings.Conversation_SpyRead) \(stringForMessageTimestamp(timestamp: readDate, dateTimeFormat: dateTimeFormat))"
        }
        if let contentsDate = SGSpyStorage.shared.date(kind: .contentsRead, peerId: peerId, messageId: message.id.id) {
            dateText = "\(dateText) \(strings.Conversation_SpyContentsRead) \(stringForMessageTimestamp(timestamp: contentsDate, dateTimeFormat: dateTimeFormat))"
        }
    }
