...
private final class StoryContainerScreenComponent: Component {
    ...
    final class View: UIView, UIGestureRecognizerDelegate {
        ...
        // MARK: Swiftgram — ghost mode: предложение включить ghost перед просмотром (1.9)
        private var isDisplayingGhostModeSuggestion: Bool = false
        ...
        func update(component: StoryContainerScreenComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<ViewControllerComponentContainer.Environment>, transition: ComponentTransition) -> CGSize {
            ...
            if self.component == nil {
                ...
                // MARK: Swiftgram — ghost mode: suggestion перед просмотром (1.9)
                // AyuGram: window_controller.cpp:598-601 — suggest && sendReadStories && !locked && !active
                let ghostSettings = SGSimpleSettings.shared
                let shouldSuggestGhostMode = ghostSettings.ghostModeSuggestBeforeStory && ghostSettings.ghostModeSendReadStories && !ghostSettings.isGhostModeEnabled
                if shouldSuggestGhostMode && !self.isDisplayingGhostModeSuggestion {
                    self.isDisplayingGhostModeSuggestion = true
                    self.isDisplayingStoriesWarning = false
                    if update {
                        self.state?.updated(transition: .immediate)
                    }
                } else if self.isDisplayingGhostModeSuggestion && !shouldSuggestGhostMode {
                    self.isDisplayingGhostModeSuggestion = false
                    if update {
                        self.state?.updated(transition: .immediate)
                    }
                }
                if self.requestedDisplayStoriesWarning && !self.isDisplayingGhostModeSuggestion {
            ...
            // MARK: Swiftgram
            // MARK: Swiftgram — ghost mode: suggestion включить ghost перед просмотром (1.9)
            if self.isDisplayingGhostModeSuggestion {
                    ...
                    component: AnyComponent(
                        SGStoryWarningComponent(
                            ...
                            action: { [weak self] in
                                ...
                                SGSimpleSettings.shared.setGhostModeEnabled(true)
                                SGSimpleSettings.shared.ghostModeDisableOnStoryClose = true
                                self.isDisplayingGhostModeSuggestion = false
                                self.state?.updated(transition: .immediate)
            ...
            } else if self.isDisplayingStoriesWarning {
                let _ = self.storiesWarning.update(
                    transition: .immediate,
                    component: AnyComponent(
                        SGStoryWarningComponent(
...
public class StoryContainerScreen: ViewControllerComponentContainer, KeyShortcutResponder {
    ...
    override public func dismiss(completion: (() -> Void)? = nil) {
        if !self.isDismissed {
            ...
            // MARK: Swiftgram — ghost mode: временный ghost на время сторис (1.9)
            // AyuGram: window_controller.cpp:610 — setDisableGhostModeOnStoryClose
            // setGhostModeEnabled(false) сам учитывает ghostModeLocked
            if SGSimpleSettings.shared.ghostModeDisableOnStoryClose {
                SGSimpleSettings.shared.ghostModeDisableOnStoryClose = false
                SGSimpleSettings.shared.setGhostModeEnabled(false)
            }
