...
func contextMenuForChatPresentationInterfaceState(chatPresentationInterfaceState: ChatPresentationInterfaceState, context: AccountContext, messages: [EngineRawMessage], controllerInteraction: ChatControllerInteraction?, selectAll: Bool, interfaceInteraction: ChatPanelInterfaceInteraction?, readStats: MessageReadStats? = nil, messageNode: ChatMessageItemView? = nil) -> Signal<ContextController.Items, NoError> {
    ...
    if messages.count == 1, let adAttribute = messages[0].adAttribute {
        ...
        if adAttribute.canReport {
            ...
            // MARK: Swiftgram — anti-delete: восстановление удалённого сообщения (AyuGram: context_menu.cpp ClearDeletedMessagesHandler / restore)
            if message.attributes.contains(where: { $0 is DeletedMessageAttribute }) {
                actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuRestoreMessage, icon: { theme in
            ...
            // MARK: Swiftgram — message shot: рендер сообщения в изображение (AyuGram: message_shot.cpp Wrapper / message_shot_box.cpp)
            if SGSimpleSettings.shared.showMessageShot, let shotMessageNode = messageNode {
                actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuMessageShot, icon: { theme in
                    return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Forward"), color: theme.actionSheet.primaryTextColor)
                }, action: { _, f in
                    f(.default)
                    if let image = ChatMessageShotRender.render(node: shotMessageNode, theme: chatPresentationInterfaceState.theme) {
                        let previewController = SGMessageShotPreviewController(image: image, theme: chatPresentationInterfaceState.theme, strings: chatPresentationInterfaceState.strings)
                        controllerInteraction.presentController(previewController, nil)
                    }
                })))
            }
        }
